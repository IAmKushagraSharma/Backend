...
@api_view(['GET'])
def get_tle(request):
    response = requests.get(f'https://tle.ivanstanojevic.me/api/tle/?api_key={api_key}').json()
    return Response(response)

@api_view(['GET'])
def tle_by_id(request):
    satId = 44804
    response = requests.get(f'https://tle.ivanstanojevic.me/api/tle/{satId}').json()
    return Response(response)
